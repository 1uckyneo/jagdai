import type { StoreShape, ValidStore, Query, Capitalize } from './types'
...
type Options<Props> = {
   ...
   */
  memo?: boolean | ((prevProps: Readonly<Props>, nextProps: Readonly<Props>) => boolean)
  /**
   * prepare for devtool in the future
   */
  name?: Capitalize
}
...
export function create<StoreType extends StoreShape, PropsType extends EmptyProps>(
  useCreateStore: (props: React.PropsWithChildren<PropsType>) => ValidStore<StoreType>,
  ...
  const StoreExecutor: React.FC<React.PropsWithChildren<PropsType>> = (props) => {
    ...
    return <StoreContext.Provider value={manager}>{props.children}</StoreContext.Provider>
  }

  const IsolationContext = createContext({})
  const IsolationProvider: React.FC<React.PropsWithChildren<unknown>> = (props) => {
    return <IsolationContext.Provider value={{}}>{props.children}</IsolationContext.Provider>
  }
  const IsolationConsumer: React.FC<React.PropsWithChildren<unknown>> = (props) => {
    useContext(IsolationContext)
    return <>{props.children}</>
  }

  const StoreProvider: React.FC<React.PropsWithChildren<PropsType>> = (props) => {
    return (
      <IsolationProvider>
        <StoreExecutor {...props}>
  ...
  const Store = options?.memo
    ? React.memo(StoreProvider, typeof options?.memo === 'function' ? options?.memo : undefined)
    : StoreProvider
  ...
  return {
    ...
    useCommand<Selection = Store['command']>(query?: Query<Store['command'], Selection>) {
      const manager = useContext(StoreContext)
